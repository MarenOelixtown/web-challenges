...
function findByLastName(employees, lastName) {
  const foundLastName = employees.find(
...
function findByCitySubString(employees, string) {
  const foundCityString = employees.find((employee) =>
    employee.city.substring()
  );
  return foundCityString;
}
...
function findByProfessionSubStringAndAge(employees, string, age) {
  const foundProfessionAndAge = employees.find(
    (employee) => employee.profession.substring() && employee.age > age
  );
  return foundProfessionAndAge;
}
