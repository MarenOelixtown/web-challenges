...
console.log(everyEmployeeHasPhoneNumber); //  employee.phone != null or employee.hasOwnProperty("phone"))
...
const everyEmployeeHasFirstAndLastName = employees.every(
  (employee) => employee.firstName != null && employee.lastName != null
);
...
const employeesSortedByAge = employees.toSorted(
  (emloyeeA, emloyeeB) => emloyeeA.age - emloyeeB.age
);
...
const employeesSortedByLastName = employees.toSorted((emloyeeA, emloyeeB) => {
  const a = emloyeeA.lastName.toLowerCase();
  const b = emloyeeB.lastName.toLowerCase();
  if (a > b) {
    return -1;
  }
  if (a < b) {
